...
from sheet_utils import get_tasks_and_rewards, sync_points_for_dates, get_total_points, get_history_for_date
...
history_map = {}
for d in dates:
    history = get_history_for_date(d.isoformat())
    history_map[d] = set(r["name"] for r in history if r["type"].lower() == "task")
...
for i, d in enumerate(dates):
    ...
    for task in tasks:
        task_name = task["name"]
        key = f"{d}-{task_name}"
        checked = (task_name in history_map[d]) if key not in st.session_state["pending_checks"] else st.session_state["pending_checks"][key]
